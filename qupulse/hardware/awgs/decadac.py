...
from qupulse.program.decadac_linspace import Command, LoopLabel, Increment, Set as _Set, Wait, LoopJmp, Play, reduce_commands, Ramp
...
#stolen and modified from qcodes driver
def _get_ramp_str(current_val:Union[float,int], final_val:Union[float,int],
                  time:float, conversion_func: Callable[:Union[float,int],int], set_update_rate_us:int) -> str:

    
    if current_val==final_val: return ""
    
    # rate = 1/time
    
    c_val = conversion_func(current_val)  # Current voltage in DAC units
    e_val = conversion_func(final_val)  # Endpoint in DAC units
    # Number of refreshes per second
    t_rate = 1e6/(set_update_rate_us) #supply update rate in MICROSECONDS
    # Number of seconds to ramp
    # secs = abs((current_val - final_val)/rate)
    secs = time
    ...
    assert abs(slope) < 2**32, 'ramp too steep'
    
    ramp_str = ""
    ramp_str += f"T{set_update_rate_us};"
    
    # Now let's set up our limits and ramp slope
    if slope > 0:
        ramp_str+=f"U{int(e_val)};"
    else:
        ramp_str+=f"L{int(e_val)};"
    
    ramp_str += f"S{slope};"
    # ramp_str += f"A{channel_select_address-3};"
...
def translate_command_list_to_ascii(commands:List[Command], channel_mapping:Union[Dict[int, int], None]=None, volt_in_dac_basis:bool=False, with_trigger:bool = True) -> str:
    ...
    last_channel_set_volts = {}
    RAMP_TIMEBASE_US = 100
    ...
    for cmd in commands:
        if cmd.__class__.__name__ == "LoopLabel":
            ...
            last_channel_set_volts[channel_mapping[cmd.channel]] = cmd.value
        ...
        elif cmd.__class__.__name__ == "Ramp":
            print('Warning: ramp expects to be after Set or other Ramp command on channel')
            last_set_value = last_channel_set_volts.get(channel_mapping[cmd.channel],None)
            assert last_set_value is not None, "Set channel before ramping"
            
            chi = channel_mapping[cmd.channel]
            bi, ci = chi//4, chi%4
            
            res.extend([
                # f"A{1545+16*(channel_mapping[cmd.channel])};", 
                f"B{bi};C{ci};", 
                _get_ramp_str(last_set_value, cmd.value,
                              cmd.ramp_time*1e-9, conversion_fn, set_update_rate_us=RAMP_TIMEBASE_US)
                ])
...
class DecaDACRepresentation:
    ...
    def upload_command_list(self, commands:List[Command], channel_mapping:Union[Dict[int, int], None]=None, volt_in_dac_basis:bool=False,
        ...
        assert resp[1:-2]==ascii_script, 'Not received what was sent'
