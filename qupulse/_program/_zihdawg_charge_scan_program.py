...
class SimpleChargeScanProgram(FixedStructureProgram):
    ...
    def wf_declarations_and_ct(self,awg,ct_tuple,ct_start_index,wf_start_index,waveforms_tuple):
        ...
        return '\n'.join(wf_decl_string), ct_idx+4, wf_idx+1, self._ct_tuple_reference, waveforms_tuple


    def update_params_in_place(self,awg,new_looped_params: Dict[str,float],
        ...
        self.parameters_scsp.update(remove_unallowed_keys(new_looped_params,allowed_keys=("start_amp_slow","start_amp_fast","end_amp_slow","end_amp_fast")))
...
# %% New enhanced chargescan program

class ChargeScanProgram(FixedStructureProgram):
    ...
    def __init__(self,
                 ...
                 measurement_channels: List[str], #["A",...]
                 awg_object: HDAWGChannelGroup,
                 dac_object: AlazarCard, #not to be confused with other meaning of dac...
                 hardware_setup: HardwareSetup,
                 inner_pt: PulseTemplate = None, #must be defined on all channels (could be made better)
                 non_looped_constant_vals: dict = {},
                 point_measurement_window: tuple = (0.1,0.9), #fractions of measurement mask at each data point
        ...
        self._inner_pt = inner_pt
        self._non_looped_constant_vals = non_looped_constant_vals
        self._channels_scsp = self.parameters_scsp["channels"].keys()
        self._awg = awg_object
        self._dac = dac_object
        self._point_measurement_window = point_measurement_window
    ...
    def _make_pt(self):
        #TODO OUTDATED NOW - need to consider every channel
        #slow_pt = PointPT()
        #TODO: why 0.5???
        #TODO: perhaps not jump but hold?
        ppt = PointPT([(0,"start_amp+i/n_steps*(end_amp-start_amp)",'jump'),("t_point","start_amp+i/n_steps*(end_amp-start_amp)",'jump'),],channel_names=["ch"],identifier="scsp_loop_step",
        ...
        slow_pts = tuple([(ppt,dict(i='i',n_steps=max(1,self.parameters_scsp['n_points_slow']-1),start_amp=self.parameters_scsp[chi]['start_amp'],dV_slow=self.parameters_scsp[chi]['dV_slow'],dV_fast=self.parameters_scsp[chi]['dV_fast']),dict(ch=chi)) for chi in self._channels_scsp.keys()])
        # fast_pts = tuple([(ppt,dict(i='j',n_steps=max(1,self.parameters_scsp['n_points_fast']-1),start_amp=self.parameters_scsp['start_amp_fast'],end_amp=self.parameters_scsp['end_amp_fast']),dict(ch=chi)) for chi in self._channels_scsp['fast']])
